...
class EmbeddingLayer(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self.num_token_types = params["type_vocab_size"]
    self.max_position_embeddings = params["max_position_embeddings"]
    self.dropout_prob = params["embedding_dropout"]
    self.word_embedding_train=params["word_embedding_train"]
    
    with tf.compat.v1.variable_scope(name):
      self.word_embeddings = tf.Variable(initial_value=np.load(params["word_embedding_path"],allow_pickle=True),
          name="word_embeddings", shape=[params["vocab_size"], params["hidden_size"]],
          dtype=tf.float32, trainable=params["word_embedding_train"])

      if params["use_token_type"]:
        self.token_type_table = tf.compat.v1.get_variable(
            "token_type_embeddings", [params["num_token_types"],params["hidden_size"]],
            dtype=tf.float32, initializer=create_initializer(
              params["initializer_range"]))
      else:
        self.token_type_table = None

      if params["use_position_embeddings"]:
        self.position_embeddings = tf.compat.v1.get_variable(
            "position_embeddings", [params["max_position_embeddings"], params["hidden_size"]],
            dtype=tf.float32, initializer=create_initializer(
              params["initializer_range"]))
      else:
        self.position_embeddings = None

      if params["use_2dposition_embedding"]:
          self.position2dx_embeddings = tf.compat.v1.get_variable(
              "position2dx_embeddings", [params["max_2dxposition"], params["hidden_size"]//2],
              dtype=tf.float32, initializer=create_initializer(
              params["initializer_range"]))

          self.position2dy_embeddings = tf.compat.v1.get_variable(
              "position2dy_embeddings", [params["max_2dyposition"], params["hidden_size"]//2],
              dtype=tf.float32, initializer=create_initializer(
              params["initializer_range"]))
      else:
          self.position2dx_embeddings=None
          self.position2dy_embeddings=None
  ...
  def call(self,
    ...
    if self.position_embeddings is not None:
      ...
      # perform a slice.
      #position_embeddings = tf.slice(self.position_embeddings, [start_pos, 0],[seq_length,self.emb_dim])
      
       position_embeddings=tf.nn.embedding_lookup(params=self.position_embeddings, ids=positionid)
      #output += tf.expand_dims(position_embeddings, axis=0)
       output +=position_embeddings

    if self.position2dx_embeddings is not None:
        tensorx= tf.nn.embedding_lookup(params=self.position2dx_embeddings, ids=todxpositionid)#batchsize,seqlength,
        tensory = tf.nn.embedding_lookup(params=self.position2dy_embeddings, ids=todypositionid)

        output += tf.concat([tensorx, tensory], axis=-1)#待会改一下这个position embedding,让他变为每个片段都是从1开始的
