...
class BertModel(tf.keras.layers.Layer):
  ...
  def __init__(self, params):
    ...
    with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
      self.embeder = utils.EmbeddingLayer(self.params)
...
#    x_2dposition=self.pad(x_2dposition)
#    y_2dposition=self.pad(y_2dposition)
    ...
    embedding_output = self.embeder(input_ids,
                                    self.params["max_encoder_length"],
                                    todxpositionid=x_2dposition,
                                    todypositionid=y_2dposition,
...
class BigbirdForTokenClassification(tf.keras.Model):
    def __init__(self, params):
        super().__init__()

        self.num_labels = params["num_labels"]
        self.bigbird = BertModel(params)
        self.dropout = tf.keras.layers.Dropout(rate=params["classifier_dropout"])
        self.classifier = tf.keras.layers.Dense(
            units=params["num_labels"],
            kernel_initializer=utils.create_initializer(params["initializer_range"]),
            name="classifier",
        )

    def call(self,input_ids,x_2dposition,y_2dposition,positionid,token_type_ids=None,training=True):
        outputs = self.bigbird(
            input_ids=input_ids,
            token_type_ids=token_type_ids,
            x_2dposition=x_2dposition,
            y_2dposition=y_2dposition,
            positionid=positionid,
            training=training,
        )
...
class CustomNonPaddingTokenLoss(tf.keras.layers.Layer):
    ...
    def call(self, y_true,y_pred,masklist):

        loss= self.loss_fn(y_true,y_pred)
        #mask = tf.cast((y_true > -1), dtype=tf.float32)
        loss = loss*masklist
        return tf.reduce_sum(loss)/tf.reduce_sum(masklist)
